...
from ultralytics.nn.modules.hbs import HBSAux, build_gt_mask
...
class BaseModel(torch.nn.Module):
    ...
    def _predict_once(self, x, profile=False, visualize=False, embed=None, return_neck=False):
        """
        Perform a forward pass through the network.
        ...
        Args:
            ...
            visualize (bool): Save the feature maps of the model if True.
            embed (list, optional): A list of feature vectors/embeddings to return.
            return_neck (bool): If True, also return the detection head's input features (neck outputs).

        Returns:
            (torch.Tensor | tuple): The last output of the model, or (output, neck_feats) if return_neck.
        """
        ...
        for m in self.model:
            ...
            if return_neck and m is self.model[-1] and isinstance(x, list):
                neck = [xi for xi in x]  # head 输入即 neck 特征(P2~P5)
            if profile:
        ...
        return (x, neck) if return_neck else x
    ...
    def loss(self, batch, preds=None):
        ...
        if preds is None:
            if hasattr(self, "hbs_aux") and self.training:
                # HBS 辅助监督：额外返回 neck 特征，用 GT mask 平滑背景后过辅助 head 算 aux loss
                preds, neck = self._predict_once(batch["img"], return_neck=True)
                masks = build_gt_mask(batch, neck)
                aux_preds = self.hbs_aux(neck, masks)
                return self.criterion(preds, batch, aux_preds=aux_preds)
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg="yolo11n.yaml", ch=3, nc=None, verbose=True):
        ...
        # HBS 辅助监督模块（训练时用 GT mask 平滑背景，仅当 yaml 显式启用 hbs: True）
        if self.yaml.get("hbs", False):
            head = self.model[-1]  # Detect
            ch_list = [head.cv2[i][0].conv.in_channels for i in range(head.nl)]  # neck 各层通道
            self.hbs_aux = HBSAux(ch_list, head.stride.tolist(), head.nc)
            if verbose:
                LOGGER.info(f"{colorstr('HBS:')} enabled on {head.nl} detection layers "
                            f"strides={head.stride.tolist()} ch={ch_list}")
